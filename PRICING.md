...
| Databases per project | 2 | 10 | unlimited |
...
**Databases per project: 2 / 10 / unlimited.** The only per-project cap; everything else counts across the org. Without it, "stuff every environment into one free project" is a complete workaround for the 1-project wall — the project cap gates isolation (own MCP URL, own policy, own tokens), and unlimited databases would sell that isolation's absence for free. Free gets 2 so the multi-database surface is honestly usable (the one-app case: app DB + analytics DB) and the *third* database — usually a second environment that belongs in its own project anyway — is the wall. Pro's 10 mirrors the machine-token posture: roomy enough that it is never the Pro→Team forcing axis.
...
- **Enforcement points.** Project create and machine-token mint each take a `SELECT … FOR UPDATE` on the `customers` row, then count usable rows under that lock (race-safe), throwing `PlanLimitError` over-cap. Database add locks the parent *project* row instead (the cap is per-project) and counts siblings under that lock; the pre-flight twin is `databaseAddBlock`, which swaps the "+ Add database" affordance for the upgrade link at the cap. Only `kind='url'` machine tokens count (`countUsableTokens`); interactive-agent (OAuth) rows mint through a separate, uncapped path, so connecting an agent never trips the cap. It surfaces as a **402** `{error:'plan_limit',…}` on the JSON API and an inline upgrade CTA on browser forms. A pure pre-flight layer (`projectCreateBlock` + `getPlanUsage`) hides doomed forms and shows `N / M` usage, but never replaces the locked check.
