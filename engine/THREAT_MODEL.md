...
- **An approval never outlives the permission.** Policy is evaluated again after a human approves, so a tightening that lands while the gate holds a write (a hot reload, a new bundle, a bundle that drops the database, a halt) denies it instead of running it on the old decision. This holds in every mode, not only gateways.
