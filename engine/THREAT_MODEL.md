...
- **Reads and writes routed through a function argument, where no table is named.** Policy rule `dangerous_function` (always on, no configuration) denies calls that reach data outside the relations the statement names. The rules above all decide from the tables a statement *names*, so a statement that carries its payload inside a function argument — `query_to_xml('SELECT ssn FROM secrets')`, `table_to_xml('secrets'::regclass)`, `database_to_xml(…)`, `pg_read_file('/etc/passwd')`, `lo_export(…)`, `dblink(…)` — named no table and was permitted, and its audit row recorded `tables_touched: []`. The XML-export family is the sharp end: it is **core PostgreSQL with `EXECUTE` granted to `PUBLIC`**, so it needs no privileged role and reads any table the connected role can see, which made it a `table_access` and `tenant_scope` bypass on an ordinary connection. Matching is on the bare function name, case-folded and recursive into nested calls, so qualification (`pg_catalog.pg_read_file`), case, and benign outer wrappers do not evade it. **Limit:** a name denylist cannot see into a `SECURITY DEFINER` UDF that calls one of these in its body — pair the rule with `REVOKE EXECUTE … FROM PUBLIC` on the role you provision.
- **PII reaching an agent that has a legitimate read.** The `column_masks` section transforms declared columns; under source rewrite (`mask_source_rewrite: true`) the rewrite happens at the source relation, so the raw value never leaves Postgres and the mask applies inside joins, filters, and aggregates rather than being scrubbed from the rows afterwards. It fails closed in both directions: result provenance the engine can't map back to a known base column denies the whole result set, and the covert-channel gate inverts `dangerous_function`'s denylist into an **allowlist** — every function a masked statement invokes must be a vetted mask-safe builtin, so an unlisted or schema-qualified call is denied. That closes the `SECURITY DEFINER` UDF hole named above, but only for databases that declare masks; a name denylist is still the floor everywhere else. Deterministic transforms are keyed by `MIDPLANE_MASK_SALT`; the engine refuses to boot with masks and no salt. This is a redaction control for values Midplane returns, **not** a substitute for Postgres column privileges — see the out-of-scope note on the customer DB role below.
...
- **Approvals are signed like policy.** In gateway mode, the control plane's answer to a held write must be signed with the same pinned key and bound to this gateway, this statement attempt and the exact SQL bytes, with a short expiry. An unsigned, mis-signed, replayed or expired "approved" is treated as "approval unavailable" and the write does not run. The read-only `check_approval` status is not signed: it never executes anything, and re-running the statement is what collects a signed answer.
- **An approval never outlives the permission.** Policy is evaluated again after a human approves, so a tightening that lands while the gate holds a write (a hot reload, a new bundle) denies it instead of running it on the old decision. If the database's engine was replaced while the write waited (new masks, a new connection, the database dropped, a halt), the write is refused outright rather than run with the old engine's masks: the agent re-runs it against the live policy. This holds in every mode, not only gateways.
...
- **`/mcp` is unauthenticated and bound to loopback only.** A gateway refuses to start on any non-loopback address, rejects any request whose `Host` (or `Origin`, when a browser sends one) is not a loopback name — so a web page on the same machine can't reach it through DNS rebinding — and ignores the `X-Midplane-Token-Id` and `X-Midplane-Scope` headers. Any process on the same host (or pod) has the access the policy allows. The authenticated front door is not built yet; do not expose the port through a proxy.
- **The state directory must be trusted for integrity, not just secrecy.** It holds the gateway's private key and, in `identity.json`, the pinned signing key and version floor. Anyone who can read it can act as the gateway toward Midplane Cloud (pull its policy, file approval requests, send heartbeats). Anyone who can *write* it can pin a key of their own and have the gateway enforce any policy they sign — the same power as holding the host, which already holds the DSNs. Mount it only into the gateway. Revoking the gateway in the dashboard cuts its link; it does not stop the process, which keeps enforcing its last policy until you stop it.
