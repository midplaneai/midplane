...
- **An approval never outlives the permission.** Policy is evaluated again after a human approves, so a tightening that lands while a write waits for a person (a hot reload, a new bundle) denies it instead of running it on the old decision. This holds in every mode, not only gateways.
