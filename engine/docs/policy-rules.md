...
Midplane ships six policy rules. The `table_access` and `guardrails` rules are
...
every other rule permitted. Masking and approvals run after all of them, on the
ALLOW path only.
...
unmasked value. Under source rewrite the same posture covers functions, by
inverting `dangerous_function`'s denylist into an **allowlist**: every function
a masked statement invokes must be a vetted mask-safe builtin, and an unlisted
or schema-qualified call is denied. That is what closes the wrapper gap
`dangerous_function` names as its limit — a `SECURITY DEFINER` UDF fails the
allowlist on its own name, without the engine having to see its body — but only
for a database that declares masks. `information_schema` / `pg_catalog` are
exempt (no maskable customer row values), mirroring the `table_access` /
`tenant_scope` carve-outs.
