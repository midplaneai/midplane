// SQLite audit writer — the local durable buffer.
//
// Day-0 spike finding: the runtime's own SQLite builtin (NOT better-sqlite3) —
// better-sqlite3 fails to load native bindings under Bun (Bun issue #4290), and
// a native module would also put a compiler in the path of `npx midplane`.
// openSqlite() resolves bun:sqlite or node:sqlite per runtime; see
// sqlite-driver.ts.
//
...
export class SqliteAuditWriter implements AuditWriter {
  ...
  private readSinceStmt: SqliteStatement;
  private deleteThroughStmt: SqliteStatement;

  constructor(path: string, opts: { create?: boolean } = {}) {
    // Callers like the audit CLI pass `create: false` to surface a friendly
    // error on a typo'd DB_PATH instead of silently creating an empty file.
    this.db = openSqlite(path, opts);
    this.applySchema();
  ...
  private applySchema(): void {
    ...
    if (tableExists) {
      ...
      // TABLE DROP COLUMN landed in 3.35 (March 2021) — both bun:sqlite and
      // node:sqlite ship well past that, so DROP is the cleanest path. Old
      // rows lose their agent_identity value, which was always NULL in
      // 0.1.x/0.2.x because no transport ever populated it.
      if (!this.hasColumn("audit_events", "agent_name")) {
        this.db.exec("ALTER TABLE audit_events ADD COLUMN agent_name TEXT");
      }
      if (!this.hasColumn("audit_events", "agent_version")) {
        this.db.exec("ALTER TABLE audit_events ADD COLUMN agent_version TEXT");
      }
      if (!this.hasColumn("audit_events", "agent_intent")) {
        this.db.exec("ALTER TABLE audit_events ADD COLUMN agent_intent TEXT");
      }
      if (this.hasColumn("audit_events", "agent_identity")) {
        this.db.exec("ALTER TABLE audit_events DROP COLUMN agent_identity");
      }
      ...
      if (this.hasColumn("audit_events", "intent_source")) {
        this.db.exec("ALTER TABLE audit_events DROP COLUMN intent_source");
        this.db.exec(
      ...
      if (!this.hasColumn("audit_events", "mcp_token_id")) {
        this.db.exec("ALTER TABLE audit_events ADD COLUMN mcp_token_id TEXT");
      }
    }
  ...
  _readAll(): unknown[] {
    return this.db.prepare("SELECT * FROM audit_events ORDER BY ts").all();
  }

  _count(): number {
    const row = this.db
      .prepare<{ c: number }, []>("SELECT COUNT(*) AS c FROM audit_events")
      .get();
    return row?.c ?? 0;
  }
}
...
function stripPostgresSection(ddl: string): string {
  ...
  // are commented but the SQLite DDL parser still runs over them.
