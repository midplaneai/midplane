...
// each table: one probe per action. Built cloud-side from the
// introspected table list ∪ policy entries; evaluated by the engine via
// /admin/dry-run. Nothing here executes anything.
//
...
//
// Probe set: select / insert / update / delete per table. Extensions
// are additive.
...
/** Mirror of the engine-side cap (50 tables → ≤200 probes per run).
...
/** Worst case: every capped table emits one probe per action. The
 *  dry-run route's request validator uses this so a cap or action change
 *  can't silently 400 the panel's own requests. */
export const MAX_PROBES_PER_RUN = PROBE_TABLE_CAP * PROBE_ACTIONS.length;
...
export function buildProbeMatrix(tables: readonly string[]): ProbeMatrix {
 ...
 *  a policy that never reached the engine, or a parse surprise. select is
 *  allowed at read+; writes only at read_write. */
