...
// Master-detail workspace for one project. A persistent left rail holds
// every aspect of the project — project-wide (Agents, Test, Settings)
...
// the project name is the rail header (the frame you're in), not a
...
export function ProjectRail({
  ...
  /** Project identity + (when >1 db) the database switcher. */
  header: React.ReactNode;
  panes: Record<ProjectSection, React.ReactNode>;
}) {
  const [section, setSection] = useState<ProjectSection>(initialSection);

  function select(next: ProjectSection) {
    setSection(next);
  ...
  return (
    <div className="flex gap-8">
      <aside className="sticky top-6 w-[180px] shrink-0 self-start">
        {header}
        <nav className="mt-5 flex flex-col gap-0.5" aria-label="Project">
