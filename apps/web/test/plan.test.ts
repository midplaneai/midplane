...
describe("projectQuota", () => {
  it("has room: not at cap, formats the usage line, passes the limit through", () => {
    expect(
      projectQuota({ billableProjects: 3, caps: CAPS.pro, plan: "pro" }),
    ).toEqual({
      atCap: false,
  ...
  it("the counter and the gate never disagree", () => {
    // The regression this replaces: an empty (database-less) project used to
    // clear the cap, so a Free workspace rendered "1/1 projects" beside a live
    // "New project" button that created nothing — it quietly attached the DSN
    // to the empty project instead. At the limit is at the limit now; adding a
    // database to an existing project doesn't come through this quota at all.
    for (const billableProjects of [1, 2, 5]) {
      const q = projectQuota({
        billableProjects,
        caps: CAPS.free,
        plan: "free",
      ...
      expect(q.atCap).toBe(true);
      expect(q.quotaLine).toBe(`free plan · ${billableProjects}/1 projects`);
    }
  });

  it("fresh Free (0/1) is not at cap", () => {
    // Post-seed-removal onboarding: a new customer owns nothing until they
    // create something, so the first create is always available.
    expect(
      projectQuota({ billableProjects: 0, caps: CAPS.free, plan: "free" }),
    ).toEqual({
      atCap: false,
  ...
  it("unlimited (Team) is never at cap and shows no quota line", () => {
    expect(
      projectQuota({ billableProjects: 5, caps: CAPS.team, plan: "team" }),
