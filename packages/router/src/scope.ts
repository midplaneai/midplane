...
// project_database_id with a polymorphic subject: an OAuth (client_id,
...
// credential, intersects it with the project's databases, and forwards it to
...
/** Granted subset of a project's databases: engine DB NAME → access. The
 *  X-Midplane-Scope header is keyed by name (the engine's alias), so we resolve
 *  project_database_id grants back to names here. */
...
export async function resolveScope(
  ...
  const rows = (await db
    ...
    .from(mcpScopeGrants)
    .where(
      and(subjectWhere, inArray(mcpScopeGrants.projectDatabaseId, cdbIds)),
    )) as Array<{ projectDatabaseId: string; access: McpScopeAccess }>;

  const scope: ScopeMap = new Map();
  for (const r of rows) {
    const name = nameById.get(r.projectDatabaseId);
